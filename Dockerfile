...
RUN Invoke-RestMethod -Method Get -URI https://onepubacrprod.azurecr.io/v2/public/windows/servercore/manifests/1903-amd64 -Headers @{Authorization=$(Get-Content -Path auth)} -UseBasicParsing -OutFile configValue
...
#RUN Invoke-RestMethod -Method POST -Uri "https://onepubng-prod.azure-api.net/MediaRefreshServiceFabric/ServerContainerSFSvc/ServerContainerValidation/ValidateContainerInfo" -Headers @{Authorization=$(Get-Content -Path authToken); #runTimeBuildVersion=$(Get-Content -Path realRunTimeVersion); runTimeUbr=$(Get-Content -Path realRunTime); product='Windows 10 1903';  hostUrl='onepubacrprod.azurecr.io/public'; tag='1903'} -Body $(Get-Content -Path #configValue_update3) -ContentType "application/json" -UseBasicParsing