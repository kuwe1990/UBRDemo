FROM mcr.microsoft.com/windows/nanoserver:10.0.18362.592-amd64
ARG CERT
ARG CONFIGURL
...
RUN Invoke-RestMethod -Method Get -URI https://mcr.microsoft.com/v2/windows/nanoserver/manifests/10.0.18362.592-amd64 -UseBasicParsing -OutFile configValue
...
RUN reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion" /v CurrentBuild > runTimeVersion/ServerContainerSFSvc/ServerContainerRelease/ValidateContainerInfo" -Headers @{Authorization=$(Get-Content -Path authToken); runTimeBuildVersion='18362'; runTimeUbr='592'; product='Windows 10 1903'} -Body $(Get-Content -Path configValue_update3) -ContentType "application/json" -UseBasicParsing